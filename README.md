...
这是CCS 2026论文《DoTMI: Document-Level Token-Only Membership Inference for Large Language Models》的实验代码实现。
...
论文Table 1展示了DoTMI与baselines在不同模型上的AUC对比。
...
**预期结果**：
- DoTMI (Method 1) 在Gutenberg上AUC ≈ 0.85-0.87
- DoTMI (Method 1) 在ArXiv上AUC ≈ 0.79-0.81
- 明显优于所有baselines

### 2. 强基线对比（Table 2）

论文Table 2对比了DoTMI与滑动窗口方法的对比。
...
这会生成 `efficiency_*.txt` 文件，包含详细的效率统计。
